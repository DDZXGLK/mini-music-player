    ...
    <div class="shell">
      <div class="container a-container" :class="{ 'is-txl': isLogin }">
        <div class="form">
          ...
          <span class="form_span">亲~快注册体验奇妙之旅吧！</span>
          ...
          <button class="button submit" @click="register">注 册</button>
        </div>
      </div>
      ...
      <div class="container b-container" :class="[{ 'is-txl': isLogin }, { 'is-z': isLogin }]">
        <div class="form">
          ...
          <span class="form_span">登录以体验奇妙之旅~</span>
          <input type="text" class="form_input" placeholder="UserName" v-model="username" />
          <input type="password" class="form_input" placeholder="PassWord" v-model="password" />
          <div class="form_link">忘记密码？</div>
          <button class="form_button button submit" @click="login">登 录</button>
        </div>
      </div>

      <div class="switch" :class="[{ 'is-gx': isgx }, { 'is-txr': isLogin }]">
        ...
        <div class="switch_container" :class="{ 'is-hidden': isLogin }">
          ...
          <p class="description">已经有账号了嘛，去登入账号来进入奇妙世界吧！！！</p>
          <button class="switch_button button" @click="toogle">登 录</button>
        </div>

        <div class="switch_container" :class="{ 'is-hidden': !isLogin }">
          <h2 class="title" style="letter-spacing: 0">Hello Friend！</h2>
          <p class="description">去注册一个账号，成为尊贵的粉丝会员，让我们踏入奇妙的旅途！</p>
          <button class="switch_button button" @click="toogle">注 册</button>
        </div>
      </div>
    </div>
  </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      //用户登录
      login() {
        this.$https.get('/login', { username: this.username, password: this.password }).then(res => {
          if (res.code == 200) {
      ...
      // 用户注册
      register() {
        this.$https.post('/register', { username: this.username, password: this.password }).then(res => {
          if (res.code == 200) {
            this.$message.success(res.message)
          } else {
            this.$message.error(res.message)
